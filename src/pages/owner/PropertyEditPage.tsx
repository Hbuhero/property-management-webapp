...
import type { PropertyPatchInput } from '@/schemas/property.schema';
...
export default function PropertyEditPage() {
    ...
    const [galleryRows, setGalleryRows] = useState<EditGalleryRow[]>([]);

    /** Avoid overwriting the form when React Query refetches the same listing in the background. */
    const hydratedForPropertyId = useRef<number | null>(null);
    ...
    useEffect(() => {
        hydratedForPropertyId.current = null;
    }, [id]);

    useEffect(() => {
        const d = detailQuery.data;
        if (!d || d.id !== id) return;
        if (hydratedForPropertyId.current === id) return;
        hydratedForPropertyId.current = id;
        setTitle(d.title);
        setDescription(d.description ?? '');
        setAddressLine(d.address ?? '');
        setLocationText(d.location);
        setRegionId(typeof d.region === 'number' ? d.region : '');
        setDistrictId(typeof d.district === 'number' ? d.district : '');
        setPropertyTypeId(typeof d.propertyTypeId === 'number' ? d.propertyTypeId : '');
                ...
                >
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">
                            JPG, PNG, or SVG. Up to {MAX_LISTING_GALLERY} images. Saving replaces the gallery with the
                            list below (order left to right).
                        </p>
                                ...
                                {galleryRows.map((row) => {
                                    const src =
                                        row.kind === 'existing'
                                            ...
                                            : row.previewUrl;
                                    const busy = row.kind === 'new' && row.uploading;
